...
export async function createPrintfulOrder(
  ...
  console.log(
    "Creating Printful order with charge:",
  ...
  if (paymentIntent.metadata?.cartItems) {
    try {
      ...
      console.error(
        "Error parsing cartItems from PaymentIntent metadata:",
        error
      );
    }
  ...
  if (!shippingDetails) {
    console.error(
      "No shipping details found in charge, paymentIntent, or session"
    );
  ...
  if (!response.ok) {
    console.error(
      "Failed to create Printful order. Response:",
